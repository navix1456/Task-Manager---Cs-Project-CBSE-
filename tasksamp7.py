...
def search_task():
    ...
    if priority_value and priority_value.isdigit():
        ...
        priority = 3  # Assign a default value if priority is not selected or not a valid digit
...
def calendar_dialog():
    ...
    year_combobox = ttk.Combobox(
        calendar_popup, textvariable=year, values=list(range(2021, 2030))
    )
    year_combobox.set(str(tk.datetime.now().year))
    year_combobox.grid(row=0, column=0, padx=5, pady=5)

    month_combobox = ttk.Combobox(
        calendar_popup, textvariable=month, values=list(calendar.month_abbr)[1:]
    )
    ...
    for week in cal.monthdayscalendar(
        int(year.get()), list(calendar.month_abbr).index(month.get())
    ):
        for day in week:
            if day == 0:
                ...
                ttk.Button(
                    cal_frame, text=str(day), command=lambda day=day: select_date(day)
                ).grid()
...
def export_to_csv_popup():
    ...
    if confirmation:
        export_to_csv()
        messagebox.showinfo("CSV Export", "Tasks exported to CSV successfully.")
...
def export_to_pdf_popup():
    export_to_pdf()
    messagebox.showinfo("PDF Export", "Tasks exported to PDF successfully.")
...
status_combobox = ttk.Combobox(
    root, textvariable=status_var, values=["Not Started", "In Progress", "Completed"]
)
...
export_pdf_button = tk.Button(root, text="Export to PDF", command=export_to_pdf_popup)
...
export_csv_button = tk.Button(root, text="Export to CSV", command=export_to_csv_popup)
